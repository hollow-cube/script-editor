...
- **Multi-project.** `ProjectContext` is hardcoded to a demo project today. Future `/:projectId` routing will replace the hardcode; the storage key already encodes the project id (`hc-project:<id>:workspace-v<version>`), so per-project state is naturally isolated.
