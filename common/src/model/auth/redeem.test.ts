...
describe('redeemLaunchCode', () => {
    test('ok → persists session and returns the access token + expiry', async () => {
        ...
        expect(out).toEqual({
            ...
            accessToken: 'acc-tok',
            accessExpiresAt: '2099-01-01T00:00:00Z',
            // Grant carried no map id → null (gate shows "open from in-game").
            mapId: null,
        })
        expect(await sessionStore.get('acct-1')).not.toBeNull()
    })

    test('ok → threads the granted map id through the outcome', async () => {
        const out = await redeemLaunchCode('ok-code', {
            client: fakeClient(() => Promise.resolve({ ...OK_RESPONSE, mapId: 'map-42' })),
            keyStore: await fakeKeyStore(),
            sessionStore: createMemorySessionStore(),
            clientKind: 'web',
        })
        expect(out.status).toBe('ok')
        expect(out.status === 'ok' && out.mapId).toBe('map-42')
    })
